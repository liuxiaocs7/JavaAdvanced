package com.liuxiaocs._01异常的概述和体系;
 ...
 *      Error: 错误的意思，严重错误Error，无法通过处理的错误，一旦出现，程序员就无能为力了，
 *             只能重启系统，优化项目。比如内存崩溃，JVM本省的崩溃。这个程序员无需理会
 *
 ...
 *              运行时阶段可能出现，运行时异常可以处理也可以不处理，编译阶段时不会出错的，
 *              但是运行阶段可能出现，还是建议提前处理。
 *
 * 小结：
 *      异常时程序在编译或者运行的过程中可能出现的错误
 *      异常分为2类：编译时异常，运行时异常。
 *              -- 编译时异常：继承Exception，编译阶段就报错，必须处理，否则代码不通过。
